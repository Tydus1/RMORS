...
bool chrif_gepard_ack_block(int fd)
{
	...
	if (violator_aid != 0 && (sd = map->id2sd(violator_aid)) != NULL)
	{
		...
		{
			if (sockt->session[sd->fd]->gepard_info.unique_id == unique_id)
			{
				gepard_send_info(sd->fd, GEPARD_INFO_BANNED, message_info);
				sockt->session[sd->fd]->recv_crypt.pos_1 = rand() % 255;
				sockt->session[sd->fd]->recv_crypt.pos_2 = rand() % 255;
				sockt->session[sd->fd]->recv_crypt.pos_3 = rand() % 255;
			}
		}
